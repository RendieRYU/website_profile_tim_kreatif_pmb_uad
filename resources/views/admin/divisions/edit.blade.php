...
@section('content')
<h1 class="text-3xl font-bold text-slate-800 mb-6">Edit Divisi</h1>
<div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 max-w-2xl">
    <form action="{{ route('admin.divisions.update', $division) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" value="{{ $division->name }}" class="w-full p-2 border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" required>
        </div>
        <div class="mb-6">
            <label class="block text-sm font-medium text-slate-700 mb-1">Warna (Hex)</label>
            <input type="color" name="color_hex" value="{{ $division->color_hex }}" class="w-full h-12 p-1 border border-slate-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 cursor-pointer" required>
        </div>
